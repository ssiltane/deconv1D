% Plot the results of deconv07b_genTikhonov_many_comp.m
%
...
load data/gentikhonov07b n alphavec xvec f mn recomat sig_num
ymin = min(f)-.2;
ymax = max(f)+.2;

% Loop over alpha values and plot each reconstruction
for iii = 1:length(alphavec)
    ...
    set(gca,'ytick',[0,1],'fontsize',fsize)
    axis([0 1 ymin ymax])
    title(['Generalized Tikhonov reconstruction, \alpha=',num2str(alpha),', error ',num2str(100*errn,'%0.1f'),'%'],'fontsize',fsize)
    
    
    % To save image to file, remove comment below
    %eval(['print -dtiff -r400 images/Tikh_', num2str(iii,'%03d'), '.tif'])
end
