% Plot the results of deconv10b_B111_many_comp.m
%
...
load data/deconv10b_B111_many n alphavec xvec f mn recomat sig_num
ymin = min(f)-.2;
ymax = max(f)+.2;
...
for iii = 1:length(alphavec)
    ...
    set(gca,'ytick',[0,1],'fontsize',fsize)
    axis([0 1 ymin ymax])
    title(['Besov reconstruction, \alpha=',num2str(alpha),', error ',num2str(100*errn,'%0.1f'),'%'],'fontsize',fsize)
